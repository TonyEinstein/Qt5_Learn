...
class Stock:
    def __init__(self):
        ...
        self.hour = []
        self.temperature = []
        self.label = QLabel(self.ui)  # 创建一个标签来显示鼠标信息
        self.label.setStyleSheet("background-color: white; border: 1px solid black;")
        self.label.setFixedHeight(20)  # 固定高度
        ...
        # 连接鼠标移动信号
        self.ui.historyPlot.scene().sigMouseMoved.connect(self.on_mouse_moved)
    ...
    def on_mouse_moved(self, pos):
        # 将鼠标位置转换为图中的数据坐标
        vb = self.plotItem.vb  # 获取 ViewBox
        data_point = vb.mapSceneToView(pos)  # 将场景坐标映射为数据坐标

        x_value = data_point.x()  # 获取x轴刻度值
        y_value = data_point.y()  # 获取y轴刻度值
        # print(x_value,y_value)
        # 在标签上显示坐标值
        self.label.setText(f"x: {x_value:.2f}, y: {y_value:.2f}")
        # 动态调整标签的宽度，以适应内容
        self.label.adjustSize()
        # 将标签移动到鼠标的旁边
        self.label.move(pos.x() + 0.1, pos.y() + 0.1)
